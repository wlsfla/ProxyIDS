    ...
    while IS_PROXY_RUNNING:
        try:
            ...
            print('[' + str(clientNum) + '] ' + datetime.datetime.now().strftime('%d/%b/%Y %H:%M:%S.%f') + '\n' 
                    ...
                    + '[' + str(clientNum) + '] < ' + res.getHeader('Content-Type') + ' ' + res.getHeader(
...
def main():
    ...
    # Print logs
    print('Proxy Server started on port ' + str(proxyPort) + ' at ' + datetime.datetime.now().strftime(
        '%d/%b/%Y %H:%M:%S.%f'))
    if OPT_MT:
    ...
    try:
        while True:
            ...
            print(f'[+] Client Addr : {clientAddr}')
            ...
            if OPT_MT:
                ...
                proxyThread.start()
                print('[+] Proxy server is Running...On thread')
            else:
